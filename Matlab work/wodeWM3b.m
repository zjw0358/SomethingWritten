%基于人眼视觉的半弱水印--水印嵌入(基于DCT系数，量化取整用floor，效果差于wodeWM3)
clc;
clear;
close all;
turnkey=8;                             % 选择原始图像
...
%         Deta=sum(tmp1a(1:4))/4;
        tmp1a(4:19)=floor(tmp1a(4:19)/Deta);              %量化
%          for  a=1:BC*BC
...
%          end
          for  a=1:BC*BC                                        % 中频
             if mod(tmp1a(a+3),2)==Tmp1(a)
                 tmp1a(a+3)=(tmp1a(a+3)*Deta)+Deta/2;
             else 
                 
                        if rem(tmp1a(a+3),Deta) >= Deta/2
                           tmp1a(a+3)=((tmp1a(a+3)+1)*Deta)+Deta/2;
                        else
                           tmp1a(a+3)=((tmp1a(a+3)-1)*Deta)+Deta/2;
                        end
             end
